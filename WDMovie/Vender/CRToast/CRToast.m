...
@property (nonatomic, assign) BOOL isFadeAnimation;

@property (nonatomic, assign) BOOL positionIsBottom;
...
NSString *const kCRToastNotificationIsFadeAnimation         = @"kCRToastNotificationIsFadeAnimation";
...
NSString *const kCRToastPositionBottom                      = @"kCRToastPositionBottom";
...
static BOOL                         kCRIsFadeAnimationDefault               = NO;

static BOOL                         kCRPositionIsBottom                     = NO;
...
+ (void)initialize {
    if (self == [CRToast class]) {
        ...
        kCRToastKeyClassMap = @{kCRToastNotificationTypeKey                 : NSStringFromClass([@(kCRNotificationTypeDefault) class]),
                                ...
                                kCRToastNotificationIsFadeAnimation         : NSStringFromClass([@(kCRIsFadeAnimationDefault) class]),
                                kCRToastPositionBottom                      : NSStringFromClass([@(kCRPositionIsBottom) class])};
    }
}
...
+ (void)setDefaultOptions:(NSDictionary*)defaultOptions {
    ...
    if (defaultOptions[kCRToastNotificationIsFadeAnimation])                      kCRIsFadeAnimationDefault                   = [defaultOptions[kCRToastNotificationIsFadeAnimation] boolValue];
    if (defaultOptions[kCRToastPositionBottom])                     kCRPositionIsBottom = [defaultOptions[kCRToastPositionBottom] boolValue];
}
...
- (BOOL)isFadeAnimation {
    return (_options[kCRToastNotificationIsFadeAnimation] ? [_options[kCRToastNotificationIsFadeAnimation] boolValue] : kCRIsFadeAnimationDefault);
}

- (BOOL)positionIsBottom {
    return (_options[kCRToastPositionBottom] ? [_options[kCRToastPositionBottom] boolValue] : kCRPositionIsBottom);
}
...
- (void)layoutSubviews {
    ...
    if (self.toast.subtitleText == nil) {
        //CGFloat labelWidth = [self.label labelWidthByAdjustText];
...
CRToastAnimationStepBlock CRToastOutwardAnimationsBlock(CRToastManager *weakSelf) {
    return ^{
        ...
        if (weakSelf.notification.isFadeAnimation) {
            weakSelf.notificationView.alpha = 0;
...
- (void)displayNotification:(CRToast*)notification {
    ...
    CGSize textSize = CGSizeMake(MAXFLOAT, notification.preferredHeight);
    CGRect textRect = [notification.text boundingRectWithSize:textSize
                                          options:NSStringDrawingTruncatesLastVisibleLine|NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading
                                       attributes:@{NSFontAttributeName : notification.font} context:nil];
    CGFloat textWidth = textRect.size.width+30;
    CGRect containerFrame;;
    __weak __block typeof(self) weakSelf = self;
    CRToastAnimationStepBlock inwardAnimationsBlock;
    if (notification.isFadeAnimation) {
        containerFrame = CGRectMake((SCREEN_WIDTH - textWidth)/2, SCREEN_HEIGHT/2+30, textWidth, notificationSize.height);
        if (notification.positionIsBottom) {
            containerFrame = CGRectMake((SCREEN_WIDTH - textWidth)/2, SCREEN_HEIGHT- 120, textWidth, notificationSize.height);
        }
        inwardAnimationsBlock = ^void(void) {
            weakSelf.notificationView.alpha = 1.0;
        ...
        containerFrame = CGRectMake(0, 64, notificationSize.width, notificationSize.height);
        inwardAnimationsBlock = ^void(void) {
            weakSelf.notificationView.frame = weakSelf.notificationWindow.rootViewController.view.bounds;
            weakSelf.statusBarView.frame = notification.statusBarViewAnimationFrame1;
        };
	...
	if (!kCRFrameAutoAdjustedForOrientation) {
		...
		switch (statusBarOrientation) {
			case UIInterfaceOrientationLandscapeLeft: {
				containerFrame = CGRectMake(0, 64, notificationSize.height, notificationSize.width);
				break;
			}
			case UIInterfaceOrientationLandscapeRight: {
				containerFrame = CGRectMake(CGRectGetWidth([[UIScreen mainScreen] bounds])-notificationSize.height, 64, notificationSize.height, notificationSize.width);
				break;
			}
    ...
    if (notification.isFadeAnimation){
        notificationView.frame = CGRectMake(0, 0, textWidth, 35);
